...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.longPressAction = ^(ItemTableviewCell *cell) {
        ...
        UIAlertAction *actionOK = [UIAlertAction actionWithTitle:TS("OK") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [config deleteCloudVideoFile:resource.indexFile];
...
//删除云视频消息回调
-(void)deleteCloudVideoConfigResult:(NSInteger)result
{
    [videoArray removeObjectAtIndex:deleteRow];
    [tableV reloadData];
}
