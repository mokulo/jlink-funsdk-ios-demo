...
#pragma mark - 删除云报警视频
- (void)deleteCloudVideoFile:(NSString *)alarmID{
    ChannelObject *channel = [[DeviceControl getInstance] getSelectChannel];
    //调用FUNSDK删除报警消息,alarmID为NULL或空字符串，表示清空;若有多个,以；分割
    [SVProgressHUD show];
    MC_Delete(self.MsgHandle, CSTR(channel.deviceMac),"VIDEO", CSTR(alarmID),1000);
}
...
#pragma mark 删除云视频回调
        case EMSG_MC_DeleteAlarm:
        {
            ...
            if (result < 0) {
                ...
                if ([self.delegate respondsToSelector:@selector(deleteCloudVideoConfigResult:)]) {
                    [self.delegate deleteCloudVideoConfigResult:msg->param1];
                }
            }
        }
            break;
